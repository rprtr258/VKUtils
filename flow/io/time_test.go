...
// 	start := time.Now()
// 	sleep1000ms := SleepA(1000*time.Millisecond, "a")
// 	atMost100ms := WithTimeout[string](100 * time.Millisecond)(sleep1000ms)
// 	_, err := UnsafeRunSync(atMost100ms)
...
// 	ion := Notify(100*time.Millisecond, "a", func(str string, err error) {
// 		assert.Equal(t, nil, err)

// 		notificationMoment <- time.Now()
// 	})
// 	_, err := UnsafeRunSync(ion)
// 	assert.Equal(t, nil, err)
